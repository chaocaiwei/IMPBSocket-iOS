...
final public class IpInfo : GeneratedMessage {
    public typealias BuilderType = IpInfo.Builder

    public static func == (lhs: IpInfo, rhs: IpInfo) -> Bool {
        if lhs === rhs {
            return true
        }
        var fieldCheck:Bool = (lhs.hashValue == rhs.hashValue)
        fieldCheck = fieldCheck && (lhs.hasOriginIp == rhs.hasOriginIp) && (!lhs.hasOriginIp || lhs.originIp == rhs.originIp)
        fieldCheck = fieldCheck && (lhs.hasOriginPort == rhs.hasOriginPort) && (!lhs.hasOriginPort || lhs.originPort == rhs.originPort)
        fieldCheck = fieldCheck && (lhs.hasObserveIp == rhs.hasObserveIp) && (!lhs.hasObserveIp || lhs.observeIp == rhs.observeIp)
        fieldCheck = fieldCheck && (lhs.hasObservePort == rhs.hasObservePort) && (!lhs.hasObservePort || lhs.observePort == rhs.observePort)
        fieldCheck = (fieldCheck && (lhs.unknownFields == rhs.unknownFields))
        return fieldCheck
    }

    /// 本机获取到的ip
    public fileprivate(set) var originIp:String! = nil
    public fileprivate(set) var hasOriginIp:Bool = false

    /// 本机获取到的端口
    public fileprivate(set) var originPort:Int32! = nil
    public fileprivate(set) var hasOriginPort:Bool = false

    /// 服务器观测到的ip
    public fileprivate(set) var observeIp:String! = nil
    public fileprivate(set) var hasObserveIp:Bool = false

    /// 服务器观测到的连接端口
    public fileprivate(set) var observePort:Int32! = nil
    public fileprivate(set) var hasObservePort:Bool = false
    ...
    override public func writeTo(codedOutputStream: CodedOutputStream) throws {
        if hasOriginIp {
            try codedOutputStream.writeString(fieldNumber: 1, value:originIp)
        }
        if hasOriginPort {
            try codedOutputStream.writeInt32(fieldNumber: 2, value:originPort)
        }
        if hasObserveIp {
            try codedOutputStream.writeString(fieldNumber: 3, value:observeIp)
        }
        if hasObservePort {
            try codedOutputStream.writeInt32(fieldNumber: 4, value:observePort)
        }
        try unknownFields.writeTo(codedOutputStream: codedOutputStream)
    }
    override public func serializedSize() -> Int32 {
        ...
        serialize_size = 0
        if hasOriginIp {
            serialize_size += originIp.computeStringSize(fieldNumber: 1)
        }
        if hasOriginPort {
            serialize_size += originPort.computeInt32Size(fieldNumber: 2)
        }
        if hasObserveIp {
            serialize_size += observeIp.computeStringSize(fieldNumber: 3)
        }
        if hasObservePort {
            serialize_size += observePort.computeInt32Size(fieldNumber: 4)
        }
        serialize_size += unknownFields.serializedSize()
        memoizedSerializedSize = serialize_size
        return serialize_size
    }
    public class func getBuilder() -> IpInfo.Builder {
        return IpInfo.classBuilder() as! IpInfo.Builder
    }
    public func getBuilder() -> IpInfo.Builder {
        return classBuilder() as! IpInfo.Builder
    }
    override public class func classBuilder() -> ProtocolBuffersMessageBuilder {
    ...
    public func toBuilder() throws -> IpInfo.Builder {
        return try IpInfo.builderWithPrototype(prototype:self)
    }
    public class func builderWithPrototype(prototype:IpInfo) throws -> IpInfo.Builder {
        return try IpInfo.Builder().mergeFrom(other:prototype)
    }
    override public func encode() throws -> Dictionary<String,Any> {
        try isInitialized()
        var jsonMap:Dictionary<String,Any> = Dictionary<String,Any>()
        if hasOriginIp {
            jsonMap["originIp"] = originIp
        }
        if hasOriginPort {
            jsonMap["originPort"] = Int(originPort)
        }
        if hasObserveIp {
            jsonMap["observeIp"] = observeIp
        }
        if hasObservePort {
            jsonMap["observePort"] = Int(observePort)
        }
        return jsonMap
    }
    override class public func decode(jsonMap:Dictionary<String,Any>) throws -> IpInfo {
        return try IpInfo.Builder.decodeToBuilder(jsonMap:jsonMap).build()
    }
    override class public func fromJSON(data:Data) throws -> IpInfo {
        return try IpInfo.Builder.fromJSONToBuilder(data:data).build()
    }
    override public func getDescription(indent:String) throws -> String {
        var output = ""
        if hasOriginIp {
            output += "\(indent) originIp: \(originIp) \n"
        }
        if hasOriginPort {
            output += "\(indent) originPort: \(originPort) \n"
        }
        if hasObserveIp {
            output += "\(indent) observeIp: \(observeIp) \n"
        }
        if hasObservePort {
            output += "\(indent) observePort: \(observePort) \n"
        }
        output += unknownFields.getDescription(indent: indent)
        return output
    }
    override public var hashValue:Int {
        get {
            var hashCode:Int = 7
            if hasOriginIp {
                hashCode = (hashCode &* 31) &+ originIp.hashValue
            }
            if hasOriginPort {
                hashCode = (hashCode &* 31) &+ originPort.hashValue
            }
            if hasObserveIp {
                hashCode = (hashCode &* 31) &+ observeIp.hashValue
            }
            if hasObservePort {
                hashCode = (hashCode &* 31) &+ observePort.hashValue
            }
            hashCode = (hashCode &* 31) &+  unknownFields.hashValue
            return hashCode
        }
    ...
    final public class Builder : GeneratedMessageBuilder {
        fileprivate var builderResult:IpInfo = IpInfo()
        public func getMessage() -> IpInfo {
            return builderResult
        }
        ...
        public var originIp:String {
            get {
                return builderResult.originIp
            }
            set (value) {
                builderResult.hasOriginIp = true
                builderResult.originIp = value
            }
        }
        public var hasOriginIp:Bool {
            get {
                return builderResult.hasOriginIp
            }
        }
        @discardableResult
        public func setOriginIp(_ value:String) -> IpInfo.Builder {
            self.originIp = value
            return self
        }
        @discardableResult
        public func clearOriginIp() -> IpInfo.Builder{
            builderResult.hasOriginIp = false
            builderResult.originIp = nil
            return self
        }
        /// 本机获取到的端口
        public var originPort:Int32 {
            get {
                return builderResult.originPort
            }
            set (value) {
                builderResult.hasOriginPort = true
                builderResult.originPort = value
            }
        }
        public var hasOriginPort:Bool {
            get {
                return builderResult.hasOriginPort
            }
        }
        @discardableResult
        public func setOriginPort(_ value:Int32) -> IpInfo.Builder {
            self.originPort = value
            return self
        }
        @discardableResult
        public func clearOriginPort() -> IpInfo.Builder{
            builderResult.hasOriginPort = false
            builderResult.originPort = nil
            return self
        }
        /// 服务器观测到的ip
        public var observeIp:String {
            get {
                return builderResult.observeIp
            }
            set (value) {
                builderResult.hasObserveIp = true
                builderResult.observeIp = value
            }
        }
        public var hasObserveIp:Bool {
            get {
                return builderResult.hasObserveIp
            }
        }
        @discardableResult
        public func setObserveIp(_ value:String) -> IpInfo.Builder {
            self.observeIp = value
            return self
        }
        @discardableResult
        public func clearObserveIp() -> IpInfo.Builder{
            builderResult.hasObserveIp = false
            builderResult.observeIp = nil
            return self
        }
        /// 服务器观测到的连接端口
        public var observePort:Int32 {
            get {
                return builderResult.observePort
            }
            set (value) {
                builderResult.hasObservePort = true
                builderResult.observePort = value
            }
        }
        public var hasObservePort:Bool {
            get {
                return builderResult.hasObservePort
            }
        }
        @discardableResult
        public func setObservePort(_ value:Int32) -> IpInfo.Builder {
            self.observePort = value
            return self
        }
        @discardableResult
        public func clearObservePort() -> IpInfo.Builder{
            builderResult.hasObservePort = false
            builderResult.observePort = nil
            return self
        }
        override public var internalGetResult:GeneratedMessage {
            get {
        ...
        @discardableResult
        override public func clear() -> IpInfo.Builder {
            builderResult = IpInfo()
            return self
        }
        override public func clone() throws -> IpInfo.Builder {
            return try IpInfo.builderWithPrototype(prototype:builderResult)
        }
        override public func build() throws -> IpInfo {
            try checkInitialized()
            return buildPartial()
        }
        public func buildPartial() -> IpInfo {
            let returnMe:IpInfo = builderResult
            return returnMe
        }
        @discardableResult
        public func mergeFrom(other:IpInfo) throws -> IpInfo.Builder {
            if other == IpInfo() {
                return self
            }
            if other.hasOriginIp {
                originIp = other.originIp
            }
            if other.hasOriginPort {
                originPort = other.originPort
            }
            if other.hasObserveIp {
                observeIp = other.observeIp
            }
            if other.hasObservePort {
                observePort = other.observePort
            }
            try merge(unknownField: other.unknownFields)
            return self
        }
        @discardableResult
        override public func mergeFrom(codedInputStream: CodedInputStream) throws -> IpInfo.Builder {
            return try mergeFrom(codedInputStream: codedInputStream, extensionRegistry:ExtensionRegistry())
        }
        @discardableResult
        override public func mergeFrom(codedInputStream: CodedInputStream, extensionRegistry:ExtensionRegistry) throws -> IpInfo.Builder {
            let unknownFieldsBuilder:UnknownFieldSet.Builder = try UnknownFieldSet.builderWithUnknownFields(copyFrom:self.unknownFields)
            while (true) {
                let protobufTag = try codedInputStream.readTag()
                switch protobufTag {
                ...
                case 10:
                    originIp = try codedInputStream.readString()

                case 16:
                    originPort = try codedInputStream.readInt32()

                case 26:
                    observeIp = try codedInputStream.readString()

                case 32:
                    observePort = try codedInputStream.readInt32()
        ...
        class override public func decodeToBuilder(jsonMap:Dictionary<String,Any>) throws -> IpInfo.Builder {
            let resultDecodedBuilder = IpInfo.Builder()
            if let jsonValueOriginIp = jsonMap["originIp"] as? String {
                resultDecodedBuilder.originIp = jsonValueOriginIp
            }
            if let jsonValueOriginPort = jsonMap["originPort"] as? Int {
                resultDecodedBuilder.originPort = Int32(jsonValueOriginPort)
            } else if let jsonValueOriginPort = jsonMap["originPort"] as? String {
                resultDecodedBuilder.originPort = Int32(jsonValueOriginPort)!
            }
            if let jsonValueObserveIp = jsonMap["observeIp"] as? String {
                resultDecodedBuilder.observeIp = jsonValueObserveIp
            }
            if let jsonValueObservePort = jsonMap["observePort"] as? Int {
                resultDecodedBuilder.observePort = Int32(jsonValueObservePort)
            } else if let jsonValueObservePort = jsonMap["observePort"] as? String {
                resultDecodedBuilder.observePort = Int32(jsonValueObservePort)!
            }
            return resultDecodedBuilder
        }
        override class public func fromJSONToBuilder(data:Data) throws -> IpInfo.Builder {
            let jsonData = try JSONSerialization.jsonObject(with:data, options: JSONSerialization.ReadingOptions(rawValue: 0))
            guard let jsDataCast = jsonData as? Dictionary<String,Any> else {
              throw ProtocolBuffersError.invalidProtocolBuffer("Invalid JSON data")
            }
            return try IpInfo.Builder.decodeToBuilder(jsonMap:jsDataCast)
        }
    }
...
final public class UserInfo : GeneratedMessage {
    ...
    public static func == (lhs: UserInfo, rhs: UserInfo) -> Bool {
        ...
        fieldCheck = fieldCheck && (lhs.hasIpInfo == rhs.hasIpInfo) && (!lhs.hasIpInfo || lhs.ipInfo == rhs.ipInfo)
        fieldCheck = (fieldCheck && (lhs.unknownFields == rhs.unknownFields))
        return fieldCheck
    }
    ...
    public fileprivate(set) var ipInfo:IpInfo!
    public fileprivate(set) var hasIpInfo:Bool = false
    required public init() {
        super.init()
    }
    override public func isInitialized() throws {
    ...
    override public func writeTo(codedOutputStream: CodedOutputStream) throws {
        ...
        if hasIp {
            try codedOutputStream.writeString(fieldNumber: 5, value:ip)
        }
        if hasIpInfo {
            try codedOutputStream.writeMessage(fieldNumber: 6, value:ipInfo)
        }
        try unknownFields.writeTo(codedOutputStream: codedOutputStream)
    }
    override public func serializedSize() -> Int32 {
        ...
        if hasIp {
            serialize_size += ip.computeStringSize(fieldNumber: 5)
        }
        if hasIpInfo {
            if let varSizeipInfo = ipInfo?.computeMessageSize(fieldNumber: 6) {
                serialize_size += varSizeipInfo
            }
        }
        serialize_size += unknownFields.serializedSize()
        memoizedSerializedSize = serialize_size
    ...
    override public func encode() throws -> Dictionary<String,Any> {
        ...
        if hasIpInfo {
            jsonMap["ipInfo"] = try ipInfo.encode()
        }
        return jsonMap
    }
    ...
    override public func getDescription(indent:String) throws -> String {
        ...
        if hasIpInfo {
            output += "\(indent) ipInfo {\n"
            if let outDescIpInfo = ipInfo {
                output += try outDescIpInfo.getDescription(indent: "\(indent)  ")
            }
            output += "\(indent) }\n"
        }
        output += unknownFields.getDescription(indent: indent)
    ...
    override public var hashValue:Int {
        get {
            ...
            if hasIpInfo {
                if let hashValueipInfo = ipInfo?.hashValue {
                    hashCode = (hashCode &* 31) &+ hashValueipInfo
                }
            }
            hashCode = (hashCode &* 31) &+  unknownFields.hashValue
            return hashCode
    ...
    final public class Builder : GeneratedMessageBuilder {
        ...
        @discardableResult
        public func setUid(_ value:UInt32) -> UserInfo.Builder {
            self.uid = value
            return self
        }
        @discardableResult
        ...
        @discardableResult
        public func setIp(_ value:String) -> UserInfo.Builder {
            self.ip = value
            return self
        }
        @discardableResult
        public func clearIp() -> UserInfo.Builder{
        ...
        public var ipInfo:IpInfo! {
            get {
                if ipInfoBuilder_ != nil {
                    builderResult.ipInfo = ipInfoBuilder_.getMessage()
                }
                return builderResult.ipInfo
            }
            set (value) {
                builderResult.hasIpInfo = value != nil
                builderResult.ipInfo = value
            }
        }
        public var hasIpInfo:Bool {
            get {
                return builderResult.hasIpInfo
            }
        }
        fileprivate var ipInfoBuilder_:IpInfo.Builder! {
            didSet {
                builderResult.hasIpInfo = true
            }
        }
        public func getIpInfoBuilder() -> IpInfo.Builder {
            if ipInfoBuilder_ == nil {
                ipInfoBuilder_ = IpInfo.Builder()
                builderResult.ipInfo = ipInfoBuilder_.getMessage()
                if ipInfo != nil {
                    try! ipInfoBuilder_.mergeFrom(other: ipInfo)
                }
            }
            return ipInfoBuilder_
        }
        @discardableResult
        public func setIpInfo(_ value:IpInfo!) -> UserInfo.Builder {
            self.ipInfo = value
            return self
        }
        @discardableResult
        public func mergeIpInfo(value:IpInfo) throws -> UserInfo.Builder {
            if builderResult.hasIpInfo {
                builderResult.ipInfo = try IpInfo.builderWithPrototype(prototype:builderResult.ipInfo).mergeFrom(other: value).buildPartial()
            } else {
                builderResult.ipInfo = value
            }
            builderResult.hasIpInfo = true
        ...
        @discardableResult
        public func clearIpInfo() -> UserInfo.Builder {
            ipInfoBuilder_ = nil
            builderResult.hasIpInfo = false
            builderResult.ipInfo = nil
            return self
        }
        override public var internalGetResult:GeneratedMessage {
            get {
        ...
        public func mergeFrom(other:UserInfo) throws -> UserInfo.Builder {
            ...
            if (other.hasIpInfo) {
                try mergeIpInfo(value: other.ipInfo)
            }
            try merge(unknownField: other.unknownFields)
            return self
        }
        ...
        override public func mergeFrom(codedInputStream: CodedInputStream, extensionRegistry:ExtensionRegistry) throws -> UserInfo.Builder {
            ...
            while (true) {
                ...
                case 50:
                    let subBuilder:IpInfo.Builder = IpInfo.Builder()
                    if hasIpInfo {
                        try subBuilder.mergeFrom(other: ipInfo)
                    }
                    try codedInputStream.readMessage(builder: subBuilder, extensionRegistry:extensionRegistry)
                    ipInfo = subBuilder.buildPartial()
        ...
        class override public func decodeToBuilder(jsonMap:Dictionary<String,Any>) throws -> UserInfo.Builder {
            ...
            if let jsonValueIpInfo = jsonMap["ipInfo"] as? Dictionary<String,Any> {
                resultDecodedBuilder.ipInfo = try IpInfo.Builder.decodeToBuilder(jsonMap:jsonValueIpInfo).build()
...
extension MsgHeader: GeneratedMessageProtocol {
    ...
    public class func parseFrom(data: Data) throws -> MsgHeader {
        return try MsgHeader.Builder().mergeFrom(data: data, extensionRegistry:BaseRoot.default.extensionRegistry).build()
    }
...
extension ExtKeyInfo: GeneratedMessageProtocol {
    ...
    public class func parseFrom(data: Data) throws -> ExtKeyInfo {
        return try ExtKeyInfo.Builder().mergeFrom(data: data, extensionRegistry:BaseRoot.default.extensionRegistry).build()
    }
...
extension Root: GeneratedMessageProtocol {
    ...
    public class func parseFrom(data: Data) throws -> Root {
        return try Root.Builder().mergeFrom(data: data, extensionRegistry:BaseRoot.default.extensionRegistry).build()
    }
...
extension IpInfo: GeneratedMessageProtocol {
    public class func parseArrayDelimitedFrom(inputStream: InputStream) throws -> Array<IpInfo> {
        var mergedArray = Array<IpInfo>()
        while let value = try parseDelimitedFrom(inputStream: inputStream) {
          mergedArray.append(value)
        }
        return mergedArray
    }
    public class func parseDelimitedFrom(inputStream: InputStream) throws -> IpInfo? {
        return try IpInfo.Builder().mergeDelimitedFrom(inputStream: inputStream)?.build()
    }
    public class func parseFrom(data: Data) throws -> IpInfo {
        return try IpInfo.Builder().mergeFrom(data: data, extensionRegistry:BaseRoot.default.extensionRegistry).build()
    }
    public class func parseFrom(data: Data, extensionRegistry:ExtensionRegistry) throws -> IpInfo {
        return try IpInfo.Builder().mergeFrom(data: data, extensionRegistry:extensionRegistry).build()
    }
    public class func parseFrom(inputStream: InputStream) throws -> IpInfo {
        return try IpInfo.Builder().mergeFrom(inputStream: inputStream).build()
    }
    public class func parseFrom(inputStream: InputStream, extensionRegistry:ExtensionRegistry) throws -> IpInfo {
        return try IpInfo.Builder().mergeFrom(inputStream: inputStream, extensionRegistry:extensionRegistry).build()
    }
    public class func parseFrom(codedInputStream: CodedInputStream) throws -> IpInfo {
        return try IpInfo.Builder().mergeFrom(codedInputStream: codedInputStream).build()
    }
    public class func parseFrom(codedInputStream: CodedInputStream, extensionRegistry:ExtensionRegistry) throws -> IpInfo {
        return try IpInfo.Builder().mergeFrom(codedInputStream: codedInputStream, extensionRegistry:extensionRegistry).build()
    }
    public subscript(key: String) -> Any? {
        switch key {
        case "originIp": return self.originIp
        case "originPort": return self.originPort
        case "observeIp": return self.observeIp
        case "observePort": return self.observePort
        default: return nil
        }
    }
}
extension IpInfo.Builder: GeneratedMessageBuilderProtocol {
    public typealias GeneratedMessageType = IpInfo
    public subscript(key: String) -> Any? {
        get { 
            switch key {
            case "originIp": return self.originIp
        ...
        set (newSubscriptValue) { 
            ...
            case "originIp":
                ...
                self.originIp = newSubscriptValue
            case "originPort":
                guard let newSubscriptValue = newSubscriptValue as? Int32 else {
                    return
                }
                self.originPort = newSubscriptValue
            case "observeIp":
                guard let newSubscriptValue = newSubscriptValue as? String else {
                    return
                }
                self.observeIp = newSubscriptValue
            case "observePort":
                guard let newSubscriptValue = newSubscriptValue as? Int32 else {
                    return
                }
                self.observePort = newSubscriptValue
            default: return
            }
        }
    }
...
extension UserInfo: GeneratedMessageProtocol {
    ...
    public class func parseFrom(data: Data) throws -> UserInfo {
        return try UserInfo.Builder().mergeFrom(data: data, extensionRegistry:BaseRoot.default.extensionRegistry).build()
    }
    ...
    public subscript(key: String) -> Any? {
        ...
        case "ipInfo": return self.ipInfo
        default: return nil
        }
    }
}
extension UserInfo.Builder: GeneratedMessageBuilderProtocol {
    ...
    public subscript(key: String) -> Any? {
        ...
        set (newSubscriptValue) { 
            ...
            case "ipInfo":
                guard let newSubscriptValue = newSubscriptValue as? IpInfo else {
                    return
                }
                self.ipInfo = newSubscriptValue
            default: return
            }
        }
    }
